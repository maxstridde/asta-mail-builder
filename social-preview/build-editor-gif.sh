...
APP_URL="${APP_URL:-http://localhost:5173/asta-mail-builder/}"
...
PAD_COLOR="#690F21"  # AStA brand red, matches the social preview
...
if ! curl -sf -o /dev/null "$APP_URL"; then
  ...
  ( cd .. && npm run dev >/tmp/asta-dev.log 2>&1 & echo $! >/tmp/asta-dev.pid )
  STARTED_DEV=1
...
if [[ -n "$STARTED_DEV" && -f /tmp/asta-dev.pid ]]; then
  kill "$(cat /tmp/asta-dev.pid)" 2>/dev/null || true
  rm -f /tmp/asta-dev.pid
fi
