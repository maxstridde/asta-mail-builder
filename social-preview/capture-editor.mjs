// capture-editor.mjs — capture progressive "stages" of the Newsletter Builder
// UI for the README demo GIF. Drives the live dev server with Playwright and
// writes screenshots/01..04.png.
//
...
//
// Env:
//   APP_URL  — builder URL (default http://localhost:5173/asta-mail-builder/)
...
const APP_URL = process.env.APP_URL || 'http://localhost:5173/asta-mail-builder/';
...
await page.evaluate(() => localStorage.removeItem('asta-mail-builder'));
...
await setInput('de-title', 'Neuigkeiten vom AStA — Juli');
...
await setEditor(
  'de-intro',
  [
    'Diesen Monat: **Vollversammlung**, neue Öffnungszeiten und ein Job',
    'beim AStA. Mehr auf [asta-bonn.de](https://asta-bonn.de/de) und',
    'unserem [Instagram](https://instagram.com/asta_bonn).',
  ].join('\n'),
);
...
const tocItems = ['Vollversammlung am 15. Juli', 'Neue Öffnungszeiten', 'Job beim AStA'];
const tocInputs = await page.$$('#de-toc-list input');
